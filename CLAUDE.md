...
- **AWS Organization setup is fully applied (Phases 1–9, completed 2026-04-25).** Org/OUs/accounts, all three SCPs attached, DeploymentRole deployed in both prod and dev, Identity Center + permission sets + account assignments, SSO profiles on WSL2 (`sincerelyhers-prod`, `sincerelyhers-dev`, `sincerelyhers-prod-readonly`, `sincerelyhers-mgmt`), IAM billing access activated, multi-region CloudTrail trails in both member accounts. Authoritative status + applied IDs live in [infrastructure/org-setup/README.md](infrastructure/org-setup/README.md).
- **Amazon seller onboardings (dev).** All six sellers (`SH`, `KK`, `LLG`, `73J`, `OH`, `CO`) are onboarded to dev: SPP app authorized, refresh token stored at `sp-api/sincerely-services/{alias}/credentials`, EventBridge rule deployed, and `REPORT_PROCESSING_FINISHED` subscription created against the shared `dev-sp-api-report-ready` destination. All six **report** EventBridge rules are currently `DISABLED` (post-shake-out for SH/KK, post-3-day-trial for LLG/73J/OH/CO). These drive the reports feature, which has **not** been cut over to prod — distinct from the feed relay below.
- **Amazon `FEED_PROCESSING_FINISHED` → Odoo relay is live in prod (completed 2026-06-09).** Base, secrets, and platform stacks are deployed to `sincerelyhers` prod; all seven `sp-api/sincerely-services/*` secrets created via `DeploymentRole`; the SP-API destination + KK/LLG/CO subscriptions are active; verified end-to-end (one real feed per seller relayed HTTP 200, DLQ clean). See [platforms/amazon/CLAUDE.md → Production deployment](platforms/amazon/CLAUDE.md#production-deployment).
- **Amazon `DATA_KIOSK_QUERY_PROCESSING_FINISHED` → Odoo relay is live in prod (completed 2026-07-08).** Second instance of the notification-relay pattern (Data Kiosk SKU Economics). Prod: platform stack (`prod-DataKioskRelay` + `prod-sp-api-datakiosk-ready` queue/DLQ, ESM enabled) deployed; nine new secrets created via `DeploymentRole` (six `sp-api/sincerely-services/{alias}/webhooks/amazon-datakiosk` + `73J/OH/SH` per-seller `credentials`); prod SP-API destination `cb91dc51-286a-48fe-b943-7c902dfa5ad2` created and all six `DATA_KIOSK_QUERY_PROCESSING_FINISHED` subscriptions **re-pointed** from the dev queue to it; verified end-to-end — real SH query-finished notification (`queryId 935126020642`) relayed HTTP 200 to `https://sh-3.sincerelyapps.com/webhook/amazon-datakiosk-sh`, DLQ clean. Dispatch is on `payload.accountId` (`amzn1.merchant.o.<merchantToken>`), **not** the feed relay's `sellerId`. Contract: [docs/handoffs/datakiosk_query_finished_webhook.md](docs/handoffs/datakiosk_query_finished_webhook.md).
  - **Re-point gotcha:** dev and prod share one SPP app, so notification subscriptions are app-global with one per (seller × type); cutover meant `delete-subscription` then `create-subscription` per seller (dev datakiosk delivery stopped as a result). `scripts/sp_api_notifications.py` gained a `delete-subscription` command for this. The pending **reports** prod cutover will hit the same wall (dev holds all six `REPORT_PROCESSING_FINISHED` subs). Dev verification was 2026-07-06 (OH/SH relayed HTTP 200 via a Tailscale funnel).
- **Extend the feed relay to SH/73J/OH — scoped, not started (2026-07-08).** Feed relay is live in prod for KK/LLG/CO only; extending to the other three is purely additive (no re-point, no new credentials, reuses feed destination `6172efe4`). **Gated** on confirming Odoo `amazon_feed_status` tolerates all feed types (`FEED_PROCESSING_FINISHED` can't be filtered by type). Full scope + work items: [docs/handoffs/feed_relay_extend_sh_73j_oh.md](docs/handoffs/feed_relay_extend_sh_73j_oh.md).

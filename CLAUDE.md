...
- **Pending Amazon seller onboardings.** `SH` (Sincerely Hers) and `KK` are onboarded to dev. `LLG`, `73J`, `OH`, `CO` are pending SPP app authorization from each seller + per-seller EventBridge rule + per-seller refresh token in Secrets Manager at `sp-api/sincerely-services/{alias}/credentials`.

...
- **Amazon seller onboardings.** All six sellers (`SH`, `KK`, `LLG`, `73J`, `OH`, `CO`) are onboarded to dev: SPP app authorized, refresh token stored at `sp-api/sincerely-services/{alias}/credentials`, EventBridge rule deployed, and `REPORT_PROCESSING_FINISHED` subscription created against the shared `dev-sp-api-report-ready` destination. All six rules are currently `DISABLED` (post-shake-out for SH/KK, post-3-day-trial for LLG/73J/OH/CO); re-enable when prod cutover begins.

...
- **Amazon `DATA_KIOSK_QUERY_PROCESSING_FINISHED` → Odoo relay verified end-to-end in dev (2026-07-06).** Second instance of the notification-relay pattern (Data Kiosk SKU Economics). In dev: platform stack (`dev-DataKioskRelay` + `dev-sp-api-datakiosk-ready` queue/DLQ), six `sp-api/sincerely-services/{alias}/webhooks/amazon-datakiosk` secrets, and the SP-API destination + all six `DATA_KIOSK_QUERY_PROCESSING_FINISHED` subscriptions are live; real OH/SH query-finished notifications relayed HTTP 200 to the Odoo dev endpoint (reached via a Tailscale funnel), DLQ drained. Dispatch is on `payload.accountId` (`amzn1.merchant.o.<merchantToken>`), **not** the feed relay's `sellerId`. Shared contract: [docs/handoffs/datakiosk_query_finished_webhook.md](docs/handoffs/datakiosk_query_finished_webhook.md). **Not** yet cut over to prod.

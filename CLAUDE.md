...
- **Amazon seller onboardings.** All six sellers (`SH`, `KK`, `LLG`, `73J`, `OH`, `CO`) are onboarded to dev: SPP app authorized, refresh token stored at `sp-api/sincerely-services/{alias}/credentials`, EventBridge rule deployed, and `REPORT_PROCESSING_FINISHED` subscription created against the shared `dev-sp-api-report-ready` destination. SH/KK rules are currently `DISABLED` (post-shake-out); LLG/73J/OH/CO rules are `ENABLED` for trial runs.

...
- **Complete AWS Organization setup.** Phases 1–6 done (org/OUs/accounts, all three SCPs attached, DeploymentRole bootstrapped in prod). Still pending: Phase 7 (IAM Identity Center + permission sets + account assignments), Phase 8 (IAM billing access per root), Phase 9 (CloudTrail all-regions per member). After Phase 7, capture Identity Center portal URL and run `aws configure sso` for profiles `sincerelyhers-prod` and `sincerelyhers-dev` on this machine. Authoritative status + applied IDs live in [infrastructure/org-setup/README.md](infrastructure/org-setup/README.md).

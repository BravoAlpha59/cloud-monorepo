...
- `requirements.txt` files inside Lambda directories are build artifacts generated by `uv export --frozen --no-dev`; gitignored and regenerated by `make build`.
...
- **Complete AWS Organization setup.** Phases 1–5 done. Phase 6 in progress: `RegionLockdown` and `ProtectCloudTrail` attached to `sincerelyhers-internal`; `ProtectProductionSecrets` not yet attached (waits on `DeploymentRole`). Still pending: Phase 7 (IAM Identity Center + permission sets + account assignments), Phase 8 (IAM billing access per root), Phase 9 (CloudTrail all-regions per member). After Phase 7, capture Identity Center portal URL and run `aws configure sso` for profiles `sincerelyhers-prod` and `sincerelyhers-dev` on this machine. Authoritative status + applied IDs live in [infrastructure/org-setup/README.md](infrastructure/org-setup/README.md).
- **Deploy `DeploymentRole` in prod**, then attach `ProtectProductionSecrets`. Template ready at [infrastructure/org-setup/deployment-role.yaml](infrastructure/org-setup/deployment-role.yaml); bootstrap command and ordering caveat in the org-setup README.

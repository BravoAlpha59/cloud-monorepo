...
- **Amazon platform identifier TODOs** — app name and LWA Client ID. Seller aliases are decided: `SH` (Sincerely Hers, onboarded), `KK`, `LLG`, `73J`, `OH` (pending SPP app authorization from each seller + per-seller EventBridge rule + per-seller refresh token in Secrets Manager at `sp-api/sincerely-services/{alias}/credentials`).

...
## Status: partially applied (Phases 1–5 done; 6 in progress)
...
6. **In progress.** Enable and attach SCPs:
   - ~~`RegionLockdown` → `sincerelyhers-internal` OU (`ou-b2n7-hyxkrhhl`)~~ **Done.**
   - ~~`ProtectCloudTrail` → `sincerelyhers-internal` OU~~ **Done.**
   - `ProtectProductionSecrets` → `sincerelyhers` (`637445353164`) only. **Blocked on DeploymentRole** — attach only after the role exists in prod (see below).
...
- [`scp-protect-production-secrets.json`](scp-protect-production-secrets.json) — **not yet attached.** Target is `sincerelyhers` prod account (`637445353164`) only. Denies Secrets Manager writes on `sp-api/*` except when the caller is `arn:aws:iam::637445353164:role/DeploymentRole`. Attach only after `DeploymentRole` exists (see below).

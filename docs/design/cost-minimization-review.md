...
- **Existing system of record (transient)**: order data currently lives in an **Odoo ERP PostgreSQL** instance covering all accounts. Bob clarified on 2026-04-23 that **a significant goal of this monorepo is to supplant the Odoo ERP**, not merely integrate with it. The canonical destination for normalized order/inventory data is therefore **this monorepo's own data layer** (the `shared/` Order, OrderLine, Product, Inventory, Report models named in the root CLAUDE.md), not Odoo. Odoo's role is a temporary source we will migrate off of.
  - "Downstream consumer" for the routine/webhook path is therefore phased: during parallel-run, deliveries may go to Odoo to keep ERP-of-record stable; once the monorepo's data layer is ready, deliveries cut over to it and Odoo is decommissioned. Don't design as if Odoo is a permanent destination.
  - A controlled read-only window into Odoo (replica, VPN, or PrivateLink) is valuable not just for design but for **migration validation and parallel-run reconciliation**. **Do not** propose network changes speculatively; wait until concretely needed and design the access path properly.

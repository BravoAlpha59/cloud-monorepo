...
- Future ERP replacement system — separate design effort, not yet documented. Both workflows below depend on the in-house ERP today, so the replacement timeline materially affects integration depth.
...
- ERP data ingress (file landing in AWS)
...
Excel workbook delivered to the operator's finance team each month, combining FBA inventory + AWD inventory + ERP purchase-cost data for the just-completed reporting month.

### Today's flow (Bob to fill in)

> Walk through who does what, where files land, and how the finance team consumes the workbook today.

_(to be written)_

### Target automated flow

- **Day 1 of month:** retrieve FBA inventory reports for the in-scope FBA sellers. Retrieve ERP purchase-cost data.
...
- **When all data is present:** run the Excel workbook generation script. Deliver workbook to the finance team via OneDrive (or NAS — see W1.D1).

### Sources

| Source | Today | Required for | Owner of integration |
|---|---|---|---|
| Amazon SP-API Reports (FBA all-inventory) | Already wired in dev for two seller aliases (now disabled, validated) | Day-1 fetch for the in-scope FBA sellers | This monorepo |
| Amazon SP-API Reports (AWD inventory) | Not yet implemented | Day-4-onward poll | This monorepo (new) |
| ERP purchase-cost data | File on NAS (manually retrieved today?) | Day-1 ingest | ERP / NAS-side |
...
1. **OneDrive vs NAS for delivery.** OneDrive is materially easier from Lambda (Microsoft Graph API + OAuth, mirrors SP-API auth pattern). NAS from Lambda requires VPC + on-prem network connectivity and is the more painful path. Recommend OneDrive unless the finance team requires files served from inside the corporate network.
2. **ERP integration: push or pull.** Strong recommendation: the ERP (or a NAS-side cron) drops a file at a known S3 prefix, Lambda waits for the S3 event. Avoids VPN/VPC complexity, decouples timing, mirrors how the existing third-party listing manager already works. Pull alternative (VPC Lambda + on-prem networking) is more brittle and probably wasted effort given the ERP's expected replacement.
3. **Seller scope.** Need to confirm which seller aliases are FBA-in-scope vs FBM-only for this workflow.
...
7. **Reporting-month folder naming.** Format? `2026-04` / `April 2026` / `2026-04 FBA Inventory Cost`? Where in OneDrive (root, shared folder, finance-shared)?
...
1. Today's manual process: step by step, including which role does what.
...
4. ERP purchase-cost data: file format, naming convention, NAS path today.
5. Finance team delivery: email a link to the workbook, or shared OneDrive folder is enough?
...
Each marketplace's available FBM inventory quantities stay in sync with the in-house ERP on a daily basis, automatically.

### Today's flow (Bob to confirm / expand)

- The in-house ERP produces daily available-FBM-inventory data residing on the company NAS.
- **Target (via third-party listing manager):** the listing manager retrieves the data on its own schedule. Already automated — no work needed in this workflow.
- **Amazon (FBM seller aliases):** manually pushed to SP-API today.
...
|---|---|---|---|
| Target (via third-party listing manager) | Vendor-pulled scheduled fetch | ✅ Automated | (no work) |
| Amazon (FBM seller aliases) | SP-API Feeds (`POST_INVENTORY_AVAILABILITY_DATA`) or Listings Items API | Manual | Daily automated push |
| Walmart | Walmart Marketplace bulk inventory feed | Manual | Daily automated push |

### Open decisions (W2.D)

1. **ERP ingress channel — same as W1.D2.** If push-to-S3 is chosen for W1, W2 inherits the channel: the ERP writes daily inventory to a known S3 prefix, two Lambdas (Amazon + Walmart) consume it.
...
5. **Seller scope confirmation.** Which seller aliases have FBM inventory to sync, and which are FBA-only (and therefore out of scope for W2)?
...
8. **Cadence and timing.** Daily at a specific hour? After the ERP's data file lands? Window-of-day acceptable?
...
2. Same for Walmart.
3. ERP file: format, schema, daily-naming convention, NAS path.
4. FBA / FBM mix per seller alias — clarify which carry FBM inventory.

---

## Out of scope (for this doc)

- The future ERP replacement system (separate design effort).
...
1. Re-enable + extend the SP-API report fetch for the in-scope FBA seller aliases on day-1-of-month cadence (mostly EventBridge rules + the existing `ReportRequester`).
2. Stand up the OneDrive delivery Lambda — prove out Microsoft Graph auth and folder creation in isolation.
3. Define the ERP-push contract (S3 prefix, file format, expected timing). The ERP / NAS side wires to that contract.
